class PostfixInterpreter:
    ...
    # Check if a token is a variable by checking if it is not a number, operator, and an assignment operator
    def isVariable(self, token):
        return not self.isNumber(token) and not self.isOperator(token) and not self.isAssignment(token) and token.isupper()
